...
script_path <- if (length(script_arg)) sub("^--file=", "", script_arg[[1]]) else "R/01_primary_merfish_ct_region_blmm.R"
...
config <- parse_cli_args(list(
  ...
  output_dir = Sys.getenv("ALDEX_OUTPUT_DIR", unset = "results/primary_merfish_ct_region_blmm"),
  nsample = 2000,
  n_cores = 10,
...
logger <- make_logger("primary_merfish_ct_region_blmm")
