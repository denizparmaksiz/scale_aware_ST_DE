...
script_path <- if (length(script_arg)) sub("^--file=", "", script_arg[[1]]) else "R/02_primary_merfish_ct_region_lme4.R"
...
config <- parse_cli_args(list(
  ...
  output_dir = Sys.getenv("ALDEX_OUTPUT_DIR", unset = "results/primary_merfish_ct_region_lme4"),
  nsample = 2000,
  n_cores = 10,
...
logger <- make_logger("primary_merfish_ct_region_lme4")
...
for (id in file_info$base_id) {
  ...
  output_path <- file.path(config$output_dir,
    sprintf("aldex_mem_cell_type_%s_results_%s.xlsx", id, config$output_date))
