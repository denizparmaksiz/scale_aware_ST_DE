...
script_path <- if (length(script_arg)) sub("^--file=", "", script_arg[[1]]) else "R/04_primary_merfish_celltype_blmm.R"
source(file.path(dirname(normalizePath(script_path)), "aldex_helpers.R"))

config <- parse_cli_args(list(
  input_dir = Sys.getenv("ALDEX_INPUT_DIR", unset = "all"),
  output_dir = Sys.getenv("ALDEX_OUTPUT_DIR", unset = "results/primary_merfish_celltype_blmm"),
  nsample = 2000,
  n_cores = 10,
...
logger <- make_logger("primary_merfish_celltype_blmm")
...
for (id in file_info$base_id) {
  ...
  method <- if (use_batch_re) "blmm" else "lm"
  ...
  output_path <- file.path(config$output_dir,
    sprintf("aldex_mem_cell_type_blmm_%s_results_%s.xlsx", id, config$output_date))
