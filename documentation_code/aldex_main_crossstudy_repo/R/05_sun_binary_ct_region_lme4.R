...
script_path <- if (length(script_arg)) sub("^--file=", "", script_arg[[1]]) else "R/05_sun_binary_ct_region_lme4.R"
source(file.path(dirname(normalizePath(script_path)), "aldex_helpers.R"))

config <- parse_cli_args(list(
  input_dir = Sys.getenv("ALDEX_INPUT_DIR", unset = "cross_study"),
  output_dir = Sys.getenv("ALDEX_OUTPUT_DIR", unset = "results/sun_binary_ct_region_lme4"),
  nsample = 2000,
  n_cores = 10,
  ...
  scale_sd = 0.25,
  input_date = "11072025",
  output_date = "02162026"
))
config$output_dir <- ensure_output_dir(config$output_dir)
logger <- make_logger("sun_binary_ct_region_lme4")
...
for (id in file_info$base_id) {
  ...
  formula <- ~ age_binary + volume + (1 | batch)
  ...
  logger$log(sprintf("[%s] running lme4, nsample=%d", id, config$nsample))
  fit <- aldex(filtered, formula, data = metadata, method = "lme4",
               nsample = config$nsample, scale = log_scale,
               n.cores = config$n_cores,
               return.pars = c("X", "estimate", "std.error", "p.val",
                               "p.val.adj", "logComp", "logScale", "random.effects"))
  results <- build_results_table(fit)
  output_path <- file.path(config$output_dir,
    sprintf("cross_study_ct_%s_it_results_%s.xlsx", id, config$output_date))
