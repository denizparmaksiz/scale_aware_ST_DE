...
script_path <- if (length(script_arg)) sub("^--file=", "", script_arg[[1]]) else "R/06_sun_continuous_celltype_blmm.R"
source(file.path(dirname(normalizePath(script_path)), "aldex_helpers.R"))

config <- parse_cli_args(list(
  input_dir = Sys.getenv("ALDEX_INPUT_DIR", unset = "cross_study_cont"),
  output_dir = Sys.getenv("ALDEX_OUTPUT_DIR", unset = "results/sun_continuous_celltype_blmm"),
  nsample = 1000,
  n_cores = 10,
  ...
  scale_sd = 0.25,
  input_date = "05122026",
  output_date = "05122026"
))
config$output_dir <- ensure_output_dir(config$output_dir)
logger <- make_logger("sun_continuous_celltype_blmm")
